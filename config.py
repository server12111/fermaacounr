...
def load_config() -> Config:
    ...
    if config.payout_enabled and config.payout_recipient_id <= 0:
        raise RuntimeError("PAYOUT_RECIPIENT_ID должен быть положительным Telegram ID")
    if config.payout_enabled and not config.payout_command.strip():
        raise RuntimeError("PAYOUT_COMMAND не может быть пустой при включённой автовыплате")
    return config
