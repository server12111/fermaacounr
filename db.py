...
class Database:
    ...
    async def complete_payout(self, account_id: int, chat_id: int, message_id: int) -> None:
        async with self.connect() as conn:
            await conn.execute(
                """UPDATE payout_ledger SET status='sent'
                WHERE account_id=? AND source_chat_id=? AND source_message_id=?""",
                (account_id, chat_id, message_id),
            )
            await conn.commit()

    async def release_payout(self, account_id: int, chat_id: int, message_id: int) -> None:
        """Release a claim when Telegram rejected the send, allowing a safe retry."""
        async with self.connect() as conn:
            await conn.execute(
                """DELETE FROM payout_ledger WHERE account_id=? AND source_chat_id=?
                AND source_message_id=? AND status='claimed'""",
                (account_id, chat_id, message_id),
            )
            await conn.commit()
